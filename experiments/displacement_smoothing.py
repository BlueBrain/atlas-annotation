...
- Apply the averaging on the deltas along the sagittal axis (between 3 slices: the one before,
the current one and the one after). For further information, you can check scipy documentation.
- Transform the resulting deltas back to displacement fields.
- Apply the averaged displacement fields to moving images to produce registered images.
- Saving the results.
"""
import argparse
import logging
...
from deal import deltas_to_dfs, dfs_to_deltas, load_dfs, safe_dfs
...
def main():
    ...
    logger.info("Extract the deltas (=transformation vector fields) for the registration")
    deltas = np.stack([[df[:, :, 1], df[:, :, 0]] for df in dfs])

    logger.info(f"Apply the averaging on the deltas along the sagittal axis")
    ...
    logger.info("Apply the averaged displacement fields to moving images to produce registered "
                "images")
