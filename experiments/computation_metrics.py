...
and saving them under metrics.pickle
Regarding the image similarities metrics:
- The metrics are computed on the entire images and saved under `metricname`
- The metrics are computed on the foreground part of the image (mask computed thanks the
reference images) and saved under `metricname_masked`
An intersection-over-union is also computed. To that end, a segmentation map foreground/background
is constructed based on every input image and IOU is computed on those segmentation maps.
The results are saved in a dictionary with structure:
{
metric_1: {
    'moving': [],
    'experiment_1': [],
...
- Computing image similarities for every slice on the entire images for the baseline and the
results.
- Computing image similarities for every slice on the foreground images for the baseline and the
results.
- Computing the IOU score for the foreground/background partition (check if the borders are
aligned).
"""
import argparse
import logging
...
parser.add_argument("--img-mov",
                    ...
                    help="The moving image/volume (=to warp). It is assuming the format of the "
                         "moving image is .npy")
parser.add_argument("--warped-img",
...
def main():
    ...
    for metric_name in tqdm(metric_fns):
        ...
        logger.info(f"Baseline {metric_name} mean: {np.mean(all_metrics[key]['moving'])}.")
        logger.info(f"Results {metric_name} mean: {np.mean(all_metrics[key][args.experiment_name])}.")

    logger.info("Computation of the different metrics after applying "
    ...
    for metric_name in tqdm(metric_fns):
        ...
        logger.info(f"Results {key} mean: {np.nanmean(all_metrics[key][args.experiment_name])}.")

    logger.info(f"Computation of IOU scores after applying a mask "
                f"defining the reference foreground.")
    ...
    logger.info(f"Results IOU mean: {np.mean(all_metrics['iou'][args.experiment_name])}.")
