#!/usr/bin/env python
"""2D fine atlases registration (with labels remapping) script."""
import logging
import sys
...
fixed = v3 atlas, finely merged version
moving = v2 atlas, finely merged version

Both atlases are preprocessed by remapping their region labels to a range
of consecutive range of integers [0, 1, 2, ... n_regions - 1]. Since an
intensity-based method is used for registration the logic is to minimize
the differences between the values assigned to different voxels.
"""
...
def main():
    """2D fine atlases registration (with labels remapping)."""
    # Paths
