...
- The axis dimension has to be the last one,
For example: [n_slices, height, width, 2] or [height, width, 2] or [n_slices, height, width, 3]
...
- If a moving image/volume is specified, creation of a second figure with an histogram of every
axis of the displacement field for the foreground part of the moving image/volume.
"""
import argparse
import logging
import pathlib
...
def main():
    ...
    for i in range(num_axis):
        ...
        axs[i].set_title(f'Histogram along {i} axis: min/max'
                         f' {np.min(df[:, :, :, i].flatten()):6.2f}'
                         f' /{np.max(df[:, :, :, i].flatten()):6.2f}')

    logger.info(f"Saving Picture 1...")
    ...
    if args.img_mov:
        ...
        for i in range(num_axis):
            ...
            axs[i].set_title(f'Histogram along {i} axis (w/o background): min/max'
