...
- The hierarchy can be defined thanks to a number. This ones has to be between 0 and the number
of level of hierarchy. The hierarchy 0 is the most detailed one, the last level should be the
distinction foreground/background of the images.

Steps:
- Loading input images/volumes.
- Reading brain hierarchy and parsing it.
- Creating volumes for every level of the hierarchy by unfurling the different regions of the brain.
- Computation of ANTsPY registration between images/volumes at the chosen level of hierarchy
- Applying transformation to the input volumes at the highest level of the hierarchy.
- Saving the results.
- Computation and printing the baseline misalignment (between input images/volumes) and the
results misalignment (between input reference and warping moving) at every level of the hierarchy.
"""
import argparse
import json
import logging
...
parser.add_argument("--brain-regions",
...
def main():
    """Computes registration at specific hierarchy."""
    ...
    img_path = out_path.parent / f"{out_path.stem}_hierarchy{args.hierarchy}{out_path.suffix}"
    df_path = out_path.parent / f"{out_path.stem}_hierarchy{args.hierarchy}_df{out_path.suffix}"
    logger.info(f"Saving Results at {img_path} and {df_path}...")
    np.save(img_path, img_reg)
    np.save(df_path, df)

    logger.info(f"Analysis of the results...")
